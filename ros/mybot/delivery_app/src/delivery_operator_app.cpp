...
int main(int argc, char *argv[])
{
  ...
  if (argc<4){
     ROS_ERROR("Usage: rosrun delivery_app delivery_operator_app_node [node_name] [running_mode] [agent_id]");
    return 0;
  }
  ...
  config->setAvailableAgentStatus();

  //running related config.
  config->setRunningMode(runningMode);

  if (agentId!=-1){
	  DeliveryOperator delop(agentId, nodeName);
  }
