...
*/
void armCallBack(const std_msgs::String& msg)
{
  if(armLock == false){
 cout <<" Hello :Welcome to Shake hand "<< msg.data<< "Value"<<endl;
 if(msg.data == "TAKE"){
   cout<<"I am going to Hold the Item" <<endl;
   armLock= true;
   TakeItem();
}
else if(msg.data == "GRIP"){
   cout<<"I am going to take the Item" <<endl;
     armLock= true;
 GripItem();

}
else if(msg.data == "HIDE"){
  cout<<"I am going to Hide the Item" <<endl;
   armLock= true;
  HideItem();
  ...
  ReleaseItem();
    ...
    cout<<"Arm is Performing the Task !!!!! Please Wait    "<<endl;
  }
...
//subscriber for Arbotix arm
	...
	//	ros::Subscriber armCommandsSubscriber = n.subscribe("arm_controller/command", 1, onArmCommand); // subscribing for joint states
		//ros::Subscriber commandReceiver = n.subscribe("/arbotixArmController/arm_position_topic", 1000, armCallBack); // CallBack for arm controller
